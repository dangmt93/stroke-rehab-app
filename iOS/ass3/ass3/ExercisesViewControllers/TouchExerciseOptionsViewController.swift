//
//  TouchExerciseOptionsViewController.swift
//  ass3
//
...
class TouchExerciseOptionsViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "goToTouchGameScreen" {
            if let gameScreen = segue.destination as? TouchExerciseGameViewController {
                gameScreen.touchExerciseOptions = touchExerciseOptions
            }
        }
    }
...
//        print(exerciseModeSwitch.isOn)
        if exerciseModeSwitch.isOn {
            ...
            touchExerciseOptions.exerciseMode = "freeplay"
        }
        else {
            goalModeContainer.isHidden = false
            touchExerciseOptions.exerciseMode = "goal"
        }
    }
    ...
    @IBAction func nButtonsSliderChanged(_ sender: Any) {
        ...
        touchExerciseOptions.nButtons = value
    }
    
    // Action - Random order switch
    @IBAction func randomOrderSwitched(_ sender: Any) {
        if randomOrderSwitch.isOn {
            touchExerciseOptions.randomOrder = true
        }
        else {
            touchExerciseOptions.randomOrder = false
        }
    }
    ...
    @IBAction func nextButtonIndicationSwitched(_ sender: Any) {
        if nextButtonIndicationSwitch.isOn {
            touchExerciseOptions.nextButtonIndication = true
        }
        else {
            touchExerciseOptions.nextButtonIndication = false
        }
    }
}
...
extension TouchExerciseOptionsViewController: UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        if pickerView == goalModePicker {
            ...
            if(goalModeSelected == "Repetition") {
                ...
                touchExerciseOptions.goalMode = "repetition"
            } else {
                goalTypeLabel.text = "Time Limit"
                goalUnitLabel.text = "mins"
                touchExerciseOptions.goalMode = "time"
            }
        }
        else if pickerView == goalCountPicker {
            touchExerciseOptions.goalCount = goalUnitValues[row]
        }
    }
}
